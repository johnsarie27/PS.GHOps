...
BeforeAll {
    ...
    $script:ManifestPath = Join-Path -Path $script:ProjectRoot -ChildPath 'PS.GHOps.psd1'
    $script:LoaderPath = Join-Path -Path $script:ProjectRoot -ChildPath 'PS.GHOps.psm1'
    ...
    Get-Module -Name 'PS.GHOps' | Remove-Module -Force
}

Describe -Name 'PS.GHOps module scaffold' -Fixture {
    Context -Name 'Manifest' -Fixture {
        ...
        It -Name 'names the .psm1 loader as RootModule' -Test {
            $manifest = Test-ModuleManifest -Path $script:ManifestPath -ErrorAction Stop
            $manifest.RootModule | Should -Be 'PS.GHOps.psm1'
        }
    }
    ...
    Context -Name 'Exports' -Fixture {
        It -Name 'exports exactly the functions backed by a Public/ file' -Test {
            $manifest = Test-ModuleManifest -Path $script:ManifestPath -ErrorAction Stop
            $publicFiles = Get-ChildItem -Path (Join-Path $script:ProjectRoot 'Public') -Filter '*.ps1' |
                ForEach-Object BaseName
            ($manifest.ExportedFunctions.Keys | Sort-Object) |
                Should -Be ($publicFiles | Sort-Object)
        }
    }
}
